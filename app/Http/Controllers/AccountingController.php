    ...
    public function index(Request $request)
    {
        ...
        $accountingData = (clone $ledgerQuery)
            ->select(
                ...
                DB::raw('SUM(total_amount - payable_amount)                                  as total_baki'),
                DB::raw('SUM(CASE WHEN is_hisab_completed = 0 THEN total_amount - payable_amount ELSE 0 END) as total_pending'),
                DB::raw('SUM(commission_amount)                              as total_commission'),
                DB::raw('SUM(total_amount - commission_amount)                               as total_net_profit'),
                DB::raw('SUM(CASE WHEN is_hisab_completed = 0 THEN payable_amount - commission_amount ELSE 0 END) as total_net_revenue')
            )
            ->groupBy('bus_id')
            ->get()
            ->keyBy('bus_id');
        ...
        $personalData = (clone $baseQuery)
            ...
            ->selectRaw('
                ...
                SUM(total_amount - payable_amount)     as baki,
        ...
        $commissionData = (clone $baseQuery)
            ...
            ->selectRaw('
                ...
                SUM(total_amount - commission_amount)  as net_profit,
