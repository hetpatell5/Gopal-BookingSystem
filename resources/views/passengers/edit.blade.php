    ...
    <form action="{{ route('passengers.update', $passenger->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="seat_number" id="seat_number" value="{{ old('seat_number', $passenger->seat_number) }}" class="w-full px-4 py-3 border border-gray-200 rounded-none focus:outline-none focus:ring-2 focus:ring-[#f0b44b] text-[14px] bg-gray-50 text-gray-500" required readonly>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const calculateAmounts = (source = null) => {
            if (perSeatPriceInput && totalSeatsInput && source === 'per_seat') {
                ...
                totalAmountInput.value = (seatsCount * price).toFixed(2);
            }

            const total = parseFloat(totalAmountInput.value) || 0;
            const perSeatComm = parseFloat(commPercentInput?.value) || 0;
            const totalSeatsCount = parseFloat(totalSeatsInput?.value) || 1;
            
            if (commAmountInput) {
                const commAmount = (total * (perSeatComm / 100));
