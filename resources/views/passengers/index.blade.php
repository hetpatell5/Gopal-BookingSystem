...
<div class="bg-white rounded-[10px] shadow-sm p-6 mb-8">
    <form method="GET" action="{{ route('passengers.index') }}" class="flex flex-wrap items-end gap-4">
        ...
        <div class="flex-1 min-w-[200px]">
            ...
            <select name="bus_id" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f0b44b] text-[14px]">
        ...
        <div class="flex-1 min-w-[200px]">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Name, Mobile, Seat, or Pickup..." class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f0b44b] text-[14px]">
        </div>

        <div class="flex-1 min-w-[200px]">
            <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2">Filter by Date</label>
            <input type="date" name="date" value="{{ request('date') }}" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f0b44b] text-[14px]">
        </div>

        <div class="flex-1 min-w-[200px]">
            <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2">AC / Non AC</label>
            <select name="ac_type" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f0b44b] text-[14px]">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('passengers.index') }}" class="bg-gray-100 text-gray-600 font-semibold py-2.5 px-4 rounded-lg hover:bg-gray-200 transition-colors">
                Clear
            </a>
        </div>
    </form>
...
@if(session('success'))
    <div class="mb-6 p-4 bg-[#e8f5ed] text-[#34a853] rounded-md text-sm font-semibold border border-[#bbf7d0]">
        {{ session('success') }}
    </div>
@endif

<div class="bg-white rounded-[10px] shadow-sm overflow-hidden mb-6">
    <form id="bulk-action-form" method="POST" action="{{ route('passengers.bulk_destroy') }}">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <div class="flex items-center gap-4">
                <button type="button" id="bulk-delete-btn" class="hidden px-4 py-2 bg-red-50 text-red-600 font-bold rounded-lg hover:bg-red-100 transition-colors text-[13px]">
                ...
                <thead>
                    <tr>
                        <th class="px-6 py-4 w-[50px] bg-gray-50 border-b border-gray-100">
                            <input type="checkbox" id="select-all" class="w-4 h-4 rounded border-gray-300 text-[#f0b44b] focus:ring-[#f0b44b]">
                        </th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 w-[50px]">
                            <input type="checkbox" name="passenger_ids[]" value="{{ $passenger->id }}" class="row-checkbox w-4 h-4 rounded border-gray-300 text-[#f0b44b] focus:ring-[#f0b44b]">
                        </td>
                    ...
                    <td class="px-6 py-4 text-[13px] font-semibold text-[#1c2238]">
                        <a href="{{ route('buses.show', $passenger->bus->id) }}" class="hover:text-[#f0b44b] hover:underline transition-colors">{{ $passenger->bus->name }}</a>
                    </td>
