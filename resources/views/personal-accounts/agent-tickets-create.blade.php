@extends('layouts.app')
@section('title', 'Record Agent Sale')
@section('header', 'Record Agent Sale')
...
<div class="max-w-2xl mx-auto w-full">
    ...
    <div class="bg-white shadow-sm rounded-none border border-gray-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-200 bg-[#f0b44b]">
            <h2 class="text-[16px] font-black text-[#1c2238] uppercase tracking-widest"><i class="fa-solid fa-user-tie mr-2"></i> Record Agent Sale</h2>
        </div>
        <div class="p-8">
            <form action="{{ route('agent-tickets.store') }}" method="POST" class="flex flex-col gap-5">
                @csrf
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Date</label>
                        <input type="date" name="sale_date" value="{{ date('Y-m-d') }}" required class="w-full border border-gray-300 rounded-none focus:border-[#f0b44b] focus:ring-1 focus:ring-[#f0b44b] outline-none text-[14px] px-3 py-2.5 shadow-sm">
                    </div>
                    
                    <div>
                        <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Agent Name</label>
                        <input type="text" name="agent_name" required placeholder="Enter Agent Name" class="w-full border border-gray-300 rounded-none focus:border-[#f0b44b] focus:ring-1 focus:ring-[#f0b44b] outline-none text-[14px] px-3 py-2.5 shadow-sm">
                    </div>
                </div>

                <div>
                    <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Bus Name</label>
                    <input type="text" name="bus_name" required placeholder="Enter Bus Name" class="w-full border border-gray-300 rounded-none focus:border-[#f0b44b] focus:ring-1 focus:ring-[#f0b44b] outline-none text-[14px] px-3 py-2.5 shadow-sm">
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Total Seats (Sheets)</label>
                        <input type="number" id="total_seats" name="total_seats" min="1" value="1" required class="w-full border border-gray-300 rounded-none focus:border-[#f0b44b] focus:ring-1 focus:ring-[#f0b44b] outline-none text-[14px] px-3 py-2.5 shadow-sm font-bold text-[#1c2238]">
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Seat Price ₹</label>
                        <input type="number" id="seat_price" name="seat_price" step="0.01" min="0" required placeholder="e.g. 600" class="w-full border border-gray-300 rounded-none focus:border-[#f0b44b] focus:ring-1 focus:ring-[#f0b44b] outline-none text-[14px] px-3 py-2.5 shadow-sm font-bold text-[#1c2238]">
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Commission %</label>
                        <div class="flex items-center gap-2">
                            <input type="number" id="comm_pct" name="commission_percentage" step="0.01" min="0" required placeholder="e.g. 10" class="w-full border border-gray-300 rounded-none focus:border-[#f0b44b] focus:ring-1 focus:ring-[#f0b44b] outline-none text-[14px] px-3 py-2.5 shadow-sm">
                ...
                <div class="p-6 bg-gray-50 border border-gray-200 mt-4 rounded-none">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <span class="font-bold text-[#1c2238] text-[16px]">₹ <span id="lbl_total_amt">0.00</span></span>
                        <input type="hidden" name="total_amount" id="inp_total_amt" value="0">
                    </div>
                    <div class="flex justify-between items-center mb-4">
                        <span class="text-[13px] font-bold text-gray-500 uppercase tracking-widest">Commission Amount</span>
                        <span class="font-bold text-rose-500 text-[16px]">₹ <span id="lbl_comm_amt">0.00</span></span>
                        <input type="hidden" name="commission_amount" id="inp_comm_amt" value="0">
                    </div>
                    <div class="flex justify-between items-center pt-4 border-t-2 border-gray-200">
                        ...
                        <span class="font-black text-green-600 text-[24px]">₹ <span id="lbl_net_amt">0.00</span></span>
                        <input type="hidden" name="net_amount" id="inp_net_amt" value="0">
                    </div>
                </div>
                ...
                <button type="submit" class="w-full mt-4 bg-[#1c2238] text-white font-bold text-[15px] px-4 py-4 rounded-none hover:bg-[#29324b] transition-colors shadow-sm uppercase tracking-widest">
                    <i class="fa-solid fa-check mr-2"></i> Save Agent Record
                </button>
            </form>
        </div>
    </div>
